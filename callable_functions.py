"""Defines a set of callable functions for the examples."""
...
def get_tomtom_key():
  """Internal function that reads the environment variable TOMTOM_APIKEY."""
  # This needs to happen at runtime because the main
  # module uses load_dotenv() after the functions in this module are imported.
  return os.environ.get("TOMTOM_APIKEY", "-unset-")


def get_weather_forecast(*args):
  """Fetches weather forecast for a given placename.

  Expects placename (e.g., "Chicago, IL") as the first argument. Returns a dict
  with "temperature" and "periodName", or an error dict.
  """
  if not args:
    error_msg = (
  ...
  if not tomtom_key:
    return {
        "error": (
            "TomTom API key is missing or could not be read from environment"
            " TOMTOM_APIKEY."
        )
    }

  # 1. Get Lat/Lon from TomTom
  geocode_url = f"{TOMTOM_BASE_URL}/search/2/geocode/{requests.utils.quote(placename)}.json?key={tomtom_key}"
  latitude = None
  longitude = None
  ...
  try:
    print(
        f"Fetching weather points for {latitude},{longitude} from"
        " Weather.gov..."
    )
    ...
    if response_points.status_code == 301:  # Handle redirect
      ...
      if not redirect_url:
        error_msg = (
            "Weather.gov points API redirected (301) but no Location header"
            " found."
        )
        print(error_msg)
        return {"error": error_msg}
      ...
      if not redirect_url.startswith("https://"):
        # Ensure no double slashes if redirect_url starts with /
        redirect_url = f"{WEATHER_GOV_BASE_URL}{redirect_url}"
    ...
    if not forecast_grid_data_url:
      error_msg = (
          "Could not find 'properties.forecast' URL in Weather.gov points"
          " response."
      )
      print(error_msg)
      return {"error": error_msg}
  ...
  try:
    ...
    error_msg = (
        f"Unexpected structure in Weather.gov forecast API response: {e}"
    )
    print(error_msg)
    return {"error": error_msg, "details": str(e)}
...
def get_min_scrabble_word_score(*args):
  ...
  Expects the word as the first argument. - Non-ASCII characters result in a
  score of 0. - Bonus of 1 point for each character over 9. Letter scores: (1
  point) - A, E, I, L, N, O, R, S, T, U (2 points) - D, G (3 points) - B, C, M,
  P (4 points) - F, H, V, W, Y (5 points) - K (8 points) - J, X (10 points) - Q,
  Z.
  """
  if not args:
    print(
        "Error: get_min_scrabble_word_score expects at least one argument"
        " (word)."
    )
    return 0
  word = args[0]
...
def get_is_known_word(*args):
  """Returns a bool indicating if the candidate is an actual word.

  Expects the candidate word as the first argument. Checks the online
  dictionary to determine the result.
  """
  if not args:
    print(
        "Error: get_is_known_word expects at least one argument (candidate"
        " word)."
    )
    return False
  candidate = args[0]
  ...
  try:
    ...
    if response.status_code == 200:
      # Optional. People might want to see the JSON response
      # for a known word.
      ...
      print(
          f"Unexpected status code {response.status_code} for word"
          f" '{candidate}': {response.text}"
      )
      # This will re-throw an HTTPError for other bad statuses
    ...
    # Or re-raise, depending on desired error handling for network issues
    return False
  except json.JSONDecodeError:
